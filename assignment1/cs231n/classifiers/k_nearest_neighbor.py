...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in xrange(num_test):
            for j in xrange(num_train):
                ...
                B = X[i]
                # not using broadcast
                dists[i, j] = np.sqrt(np.sum(np.square(A - B)))
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in xrange(num_test):
            ...
            # new version - fast
            A_square = np.sum(np.square(A), axis=1).reshape(
                num_train)  # (train_num , 3072)
    ...
    def compute_distances_no_loops(self, X):
        ...
        A_square = np.sum(np.square(A), axis=1).reshape(
            num_train, 1)  # (train_num , 3072)
        B_square = np.sum(np.square(B), axis=1).reshape(
            num_test, 1)  # (test_num, 3072)

        AB = np.dot(A, B.T)

        # (test_num, train_num)
        dists = np.sqrt((A_square + B_square.T - 2 * AB).T)
        #######################################################################
        #                         END OF YOUR CODE                              #
        #######################################################################
        return dists
